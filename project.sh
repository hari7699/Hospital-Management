#!/bin/bash
# Hospital databases management system using Bash Shell scripting by Krishnasree and Harihara
mkdir DBMS 2>> ./.error.log
clear
echo "Welcome To Hospital DBMS"
function mainMenu {
  echo -e "\n+---------Main Menu--------------+"
...
function q1 {
  echo -e "Enter Table Name: \c"
  read tName
  echo -e "Enter Bill amount: \c"
  read bAmount
  column -t -s '|' $tName 2>>./.error.log
  awk -F ":" '{print $1, $2, $3, $4, $5, $6}' Building;
  if [[ $? != 0 ]]
  then
